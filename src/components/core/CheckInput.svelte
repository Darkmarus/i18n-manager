...
<div>
  <div class="flex relative {className}">
    ...
    <label for="checkbox-{id}" class="ml-1 font-norma cursor-pointer select-none">{label}</label>
    <button
      aria-label="Checkbox"
      class="absolute text-white opacity-0 peer-checked:opacity-100 top-2.5 left-2.5 cursor-pointer transform -translate-x-1/2 -translate-y-1/2"
      onclick={() => {
        checked = !checked;
        onClick && onClick(checked);
      }}>
