...
<div class="flex items-center justify-center">
  <div class="flex space-x-1">
    <Button disabled={tableProvider?.pagination?.page === 1} onclick={onPrevPage}>Prev</Button>

    <div class="flex justify-center min-w-96">
      {#each pages as p}
        <Button active={p === tableProvider?.pagination?.page} disabled={p === '...'} onclick={() => onChangePage(p)}
          >{p}</Button>
      {/each}
    </div>

    <Button disabled={tableProvider?.pagination?.page === tableProvider?.pagination?.totalPages} onclick={onNextPage}
      >Next</Button>
  </div>
</div>
